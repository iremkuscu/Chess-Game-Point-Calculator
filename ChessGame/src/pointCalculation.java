...
public class pointCalculation{
	...
	public static double blackTeamPointCalculation() throws FileNotFoundException{ // method to calculate total point of black team
		double blackTeamPoint = 0;
		...
		for(int i=0; i<ChessBoard.size; i++) { //iterates over whole String chessBoard
			for(int j=0; j<ChessBoard.size; j++) {
				if(totalBoardString[i][j].charAt(1) == 's') { // only gets the black team's chess pieces' points
					...
					blackTeamPoint = blackTeamPoint + point; 
				}
			}
		}
		return blackTeamPoint;
	}
	
	public static double whiteTeamPointCalculation() throws FileNotFoundException{ // method to calculate total point of white team
		double whiteTeamPoint = 0;
		String[][] totalBoardString = ChessBoard.TextFileReader(ChessBoard.fileName);
		boardSpot[][] totalBoardObject = ChessBoard.fillChessPieceArray(ChessBoard.fileName);
		
		for(int i=0; i<ChessBoard.size; i++) { 
			for(int j=0; j<ChessBoard.size; j++) {
				if(totalBoardString[i][j].charAt(1) == 'b') { // only gets the white team's chess pieces' points
					double point = pointer(totalBoardObject[i][j]);
					whiteTeamPoint = whiteTeamPoint + point;
				}	
			}
		}
		return whiteTeamPoint;
	}
